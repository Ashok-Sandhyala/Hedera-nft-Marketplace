...
export default function ForgetPassword() {
  ...
  const handleValidate = async (event) => {
    ...
    if (!email) {
      ...
      const config = {
        ...
        Subject: "This is the subject for forget password in nft market place",
  ...
  const signupbutton = async () => {
    try {
      const response = await axios.post("http://localhost:9000/updatePassword/" + email + "/" + confirmPassword);
      console.log(response.data);
      if (response.data) {
        alert('Password update was Successful');

        const emailConfig = {
          SecureToken: "a950768f-ad90-436e-9f26-a6a03895f959",
          To: email,
          From: 'bora1132004@gmail.com',
          Subject: "Hedera NFT Password Chnaged Successful",
  ...
  />
  {/* Display a message if email does not contain "@gmail.com" */}
  {email && !email.includes("@gmail.com") && (
    <Typography variant="body2" color="error">
      Must be a valid Gmail address (e.g., example@gmail.com)
    </Typography>
  )}
</Grid>
                  <Grid item xs={12}>
